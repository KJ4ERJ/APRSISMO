...
function M:createChildren()
    ...
    self._valueLabel = TextLabel(self._value)
    --self._valueLabel:setWordBreak(MOAITextBox.WORD_BREAK_CHAR)
...
function M:updateDisplay()
    ...
    for i,v in ipairs(self._buttons) do
		...
		btn:setScl(self._scale,self._scale,1)
    ...
    self._valueLabel:setColor(unpack(self:getStyle("valueColor")))
    self._valueLabel:setTextSize(self:getStyle("valueSize"))
    self._valueLabel:setFont(self:getStyle("font"))
    local tLeft, tTop, tRight, tBottom = self._textLabel:getStringBounds(1, self._text and #self._text or 1)
    self._valueLabel:setSize(tWidth-pRight, tHeight)
    self._valueLabel:setPos(iLeft + iw + iRight + pLeft -pRight, yoff + pTop + (tBottom-tTop))
    self._valueLabel:setAlign("right", "top")
...
-- @param text text
--------------------------------------------------------------------------------
function M:setValue(text)
    self._value = text
    self._valueLabel:setText(text:gsub("<","<<"))
end
...
function M:setValueSize(points)
    self:setStyle(M.STATE_NORMAL, "valueSize", points)
end
function M:setTitleSize(points)
    self:setStyle(M.STATE_NORMAL, "titleSize", points)
end
function M:setIconScaleFactor(factor)
    self:setStyle(M.STATE_NORMAL, "iconScaleFactor", points)
end
