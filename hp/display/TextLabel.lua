...
--print("textLabel:setSize")
    width = width or self:getWidth()
    height = height or self:getHeight()
    
--print("textLabel:setSize:1")
    local left, top = self:getPos()
--print("textLabel:setSize:2")
    self:setRect(-width / 2, -height / 2, width / 2, height / 2)
--print("textLabel:setSize:3")
    self:setPos(left, top)
--print("textLabel:setSized")
end
...
function M:fitSize(length)
    ...
    local padding = 4
	if type(config) == 'table' and type(config.Screen) == 'table' and type(config.Screen.scale) == 'number' then
		local tpadding = math.floor(padding * config.Screen.scale + 0.5)
		if tpadding > padding then padding = tpadding end
	end

    local left, top, right, bottom = self:getStringBounds(1, length)
	if not bottom then
		print("TextLabel:fitSize("..tostring(length)..") getStringBounds Failed ("..tostring(self._textSizePoints).."pts)!  Using "..tostring(length).."x"..tostring(self._textSizePoints*2))
		self:setSize(length,self._textSizePoints*2)
		self.fitFailed = true
	else
		...
		if self.fitFailed then
			self.fitFailed = false
			print("TextLabel:fitSize:getStringBounds("..tostring(self._textSizePoints).."pts)="..width.."x"..height)
		end
--print("textLabel:fitSize:Invoking setSize")
		self:setSize(width, height)
--print("textLabel:fitSize:Invoked setSize")
	end
end
