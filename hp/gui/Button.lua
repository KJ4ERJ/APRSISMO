...
function M:updateDisplay()
    ...
    label:setTextSize(self._textSizePoints or self:getStyle("textSize"))
...
function M:setTextSize(points)
	...
	self._label:setTextSize(points)
end
...
function M:fitSize()
	...
	local w, h = self._label:getWidth(), self._label:getHeight()
	local textPad = 0	-- Account for fitSize issues
	self:setSize(w + paddingLeft + paddingRight + textPad, h + paddingTop + paddingBottom)
end
...
function M:touchUpHandler(e)
    ...
    e:stop()
    print('Button:touchUpHandler('..tostring(self._text)..')')
