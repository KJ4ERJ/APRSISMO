...
function M.copy(src, dest)
    dest = dest or {}
--if src.stoped ~= nil or dest.stoped ~= nil then print('table.copy:src('..tostring(src)..')='..type(src.stoped)..'('..tostring(src.stoped)..') dest('..tostring(dest)..')='..type(dest.stoped)..'('..tostring(dest.stoped)..')') end
...
-- @param t table
-- @param o element
-- @param i index (optional, end if not specified)
-- @return If it already exists, false. If you add is true.
--------------------------------------------------------------------------------
function M.insertElement(t, o, i)
