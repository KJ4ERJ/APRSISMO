...
local function openComplete()
    ...
    M:dispatchEvent(Event(Event.SCENE_RESUMING, M), currentScene)
...
print("sceneManager:onCreate "..tostring(sceneName))
    scene:onCreate(params)
print("sceneManager:onCreated "..tostring(sceneName))
    scene:setVisible(false)
    addScene(scene)
    nextScene = scene
...
-- @param params (option)Parameters that define the behavior.
-- @param onComplete (option)Callback when close Animation is complete
--------------------------------------------------------------------------------
function M:closeScene(params, onComplete)
    if transitioning then
    ...
    M:dispatchEvent(Event(Event.SCENE_CLOSING, M), closingScene)
    closingScene:onStop()
	
	animateScene(params, function ()
					closeComplete()
...
--------------------------------------------------------------------------------
-- Add to the bottom of the front layers.
-- @param layer layer
--------------------------------------------------------------------------------
function M:addBehindFrontLayer(layer)
    M:updateRender()
    return table.insertElement(frontLayers, layer, 1)	-- Insert at start of frontLayers
end
